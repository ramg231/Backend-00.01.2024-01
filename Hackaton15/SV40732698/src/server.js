...
function chat_start() {
  ...
  io.sockets.on("connection", function (socket) {
    ...
    socket.on("recoger-paquete", function (data, user) {
      ...
      var sql =
        "INSERT INTO order_amazon (username,order_user,origin, destiny , kilo) VALUES ('" +
        user +
        "' , '" +
        objPaquete.producto +
        "', '" +
    ...
    socket.on("track", function (data) {
     let objTracking = JSON.parse(data);
      var sql =
        "INSERT INTO order_amazon_tracking (origin, destiny) VALUES ( '" +
        objTracking?.from +
        "', '" +
        objTracking.to +
        "')";
...
function login(req, res) {
  ...
  var sql = "login * FROM login WHERE username='" + username + "'";
  con.query(sql, function (err, result, fields) {
