...
app.post('/update-package-status', function (req, res) {
    // Aquí deberías manejar la actualización del estado del paquete
    var packageStatus = req.body.packageStatus;
    var user = req.body.user;
    // Realiza la inserción del estado del paquete en la base de datos
    var sql = "INSERT INTO packageStatus (packageStatus , user) VALUES ('" + packageStatus + "' , '" + user + "')";
    con.query(sql, function (err, result) {
        if (err) throw err;
        // Emitir un evento a todos los clientes para notificar la actualización del estado del paquete
        io.sockets.emit('package-status-updated', { packageStatus: packageStatus, username: user });
